...
def alpha_beta_search(state, game):
    ...
    def max_value(state, alpha, beta):
        ...
        for a in game.actions(state):
            for move in game.actions(state)[a]:
                print("Max move "+game.to_move(state))
                print(a+" "+move)
                print("Game Board for max_value ")
                game.display(state)
                input()
                v = max(v, min_value(game.result(state, a+" "+move), alpha, beta))
                if v >= beta:
                    return v
                alpha = max(alpha, v)
        return v
    ...
    def min_value(state, alpha, beta):
        ...
        for a in game.actions(state):
            for move in game.actions(state)[a]:
                print("Min Move "+game.to_move(state))
                print(a+" "+move)
                print("Game Board for min_value ")
                game.display(state)
                input()
                v = min(v, max_value(game.result(state, a+" "+move), alpha, beta))
    ...
    for a in game.actions(state):
        for move in game.actions(state)[a]:
            v = min_value(game.result(state, a+" "+move), best_score, beta)
            if v > best_score:
                best_score = v
                best_action = a+" "+move
...
def alpha_beta_cutoff_search(state, game, d=7, cutoff_test=None, eval_fn=None):
    """Search game to determine best action; use alpha-beta pruning.
    ...
    def max_value(state, alpha, beta, depth):
        ...
        for a in game.actions(state):
            for move in game.actions(state)[a]:
                v = max(v, min_value(game.result(state, a+" "+move), alpha, beta, depth + 1))
                if v >= beta:
                    return v
                alpha = max(alpha, v)
        return v
    ...
    def min_value(state, alpha, beta, depth):
        ...
        for a in game.actions(state):
            for move in game.actions(state)[a]:
                v = min(v, max_value(game.result(state, a+" "+move), alpha, beta, depth + 1))
                if v <= alpha:
                    return v
                beta = min(beta, v)
        return v
    ...
    for a in game.actions(state):
        for move in game.actions(state)[a]:
            v = min_value(game.result(state, a+" "+move), best_score, beta, 1)
            if v > best_score:
                best_score = v
                best_action = a+ " "+move
    return best_action
...
def query_player(game, state):
    ...
    if game.actions(state):
        print("Move input format example: 5,0 4,1")
